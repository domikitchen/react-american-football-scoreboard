...
function App() {
  ...
  const [homeScore, newScoreH] = useState(0);
  const [awayScore, newScoreA] = useState(0);
  
  const touchDownH = evt => {
    newScoreH(homeScore + 7);
  }
  const touchDownA = evt => {
    newScoreA(awayScore + 7);
  }
  const fieldGoalH = evt => {
    newScoreH(homeScore + 3);
  }
  const fieldGoalA = evt => {
    newScoreA(awayScore + 7);
  }
